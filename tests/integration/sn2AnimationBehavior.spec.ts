...
describe('SN2 Animation Behavior Integration', () => {
  ...
  beforeEach(() => {
    ...
    (window as any).moleculeManager = {
      newMolecule: vi.fn((name: string, position: any) =>
        createMockMolecule(name, new THREE.Vector3(position.x, position.y, position.z))
      ),
      getMolecule: vi.fn(),
  ...
  describe('Atom Finding Logic', () => {
    ...
    it('correctly identifies carbon atoms', () => {
      const carbonAtom = (sn2Animation as any).findNearestCarbonTo(
        substrate,
        substrate.group.children[4]
      ); // Br atom
