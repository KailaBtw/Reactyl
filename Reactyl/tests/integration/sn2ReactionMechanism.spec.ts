...
describe('SN2 Reaction Mechanism', () => {
  ...
  describe('Leaving Group Removal', () => {
    it('removes leaving group atom from substrate', () => {
      ...
      const leavingGroupAtom = substrate.group.children.find(
        (child: any) => child.userData.element === 'Br'
      );
      ...
      setTimeout(() => {
        ...
        expect(
          substrate.group.children.find((child: any) => child.userData.element === 'Br')
        ).toBeUndefined();
      }, 300); // 30% of 1000ms
    });
  ...
  describe('Complete Reaction Sequence', () => {
    ...
    it('maintains reaction state throughout sequence', () => {
      ...
      setTimeout(() => {
        ...
        expect(substrate.group.children.some((child: any) => child.userData.element === 'C')).toBe(
          true
        );
        expect(substrate.group.children.some((child: any) => child.userData.element === 'H')).toBe(
          true
        );
      }, 1200);
    });
  ...
  describe('Error Handling', () => {
    it('handles missing leaving group atom gracefully', () => {
      ...
      substrate.group.children = substrate.group.children.filter(
        (child: any) => child.userData.element !== 'Br'
      );
    ...
    it('handles missing carbon atom gracefully', () => {
      // Remove carbon atom
      substrate.group.children = substrate.group.children.filter(
        (child: any) => child.userData.element !== 'C'
      );
