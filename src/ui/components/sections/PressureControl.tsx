  ...
  return (
    <section className="p-3 rounded-lg border border-purple-500/20 bg-gradient-to-br from-purple-50/50 to-purple-100/30 dark:from-purple-950/20 dark:to-purple-900/10">
      <div className="mb-3 flex items-center gap-2">
        <label className={`text-sm font-semibold ${themeClasses.text}`}>Pressure</label>
        <InfoBubble
...
Standard conditions: 1 atm at 25°C`}
      ...
      <div className="flex items-baseline gap-2 mb-3">
        <div className={`text-3xl font-bold ${themeClasses.text}`}>{pressure.toFixed(2)}</div>
        <div className={`text-lg font-medium text-gray-600 dark:text-gray-400`}>atm</div>
      </div>
        ...
        return (
          <div className="mb-3 text-xs">
            <div className="flex items-center justify-between mb-1">
              ...
              <span
                className={`font-medium ${pressureFactor >= 1 ? 'text-purple-600 dark:text-purple-400' : 'text-gray-500'}`}
              >
      ...
      <div className={`mt-3 text-xs italic ${themeClasses.textSecondary}`}>
        {pressure < 0.5
          ? 'Low pressure - fewer collisions'
          : pressure < 1.0
            ? 'Below standard - reduced rate'
            : pressure < 2.0
              ? 'Standard pressure - normal rate'
              : pressure < 5.0
                ? 'Elevated - increased collision frequency'
