...
export const HeroEnergyCurve: React.FC<HeroEnergyCurveProps> = ({
  ...
  const calculateEnergyProfile = (reactionCoordinate: number) => {
    ...
    if (reactionCoordinate <= 0.5) {
      ...
      return (
        ...
        (transitionStateEnergy - productEnergy) * (1 - Math.cos((t * Math.PI) / 2))
      );
    }
  };
  ...
  const kineticEnergyKJ = (0.5 * 28.0 * currentVelocity ** 2 * 6.022e23) / 1000; // Rough conversion
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
      ...
      <div className="flex justify-between items-center px-4 py-2 border-b border-gray-100">
        ...
        <div className="text-xs text-gray-500">Progress: {reactionProgress.toFixed(0)}%</div>
      </div>
        ...
        <svg width={width} height={height} className="w-full" viewBox={`0 0 ${width} ${height}`}>
          ...
          <g>
            ...
            <circle
              ...
              cy={
                height -
                padding -
                ((reactantEnergy - minEnergy) / energyRange) * (height - 2 * padding)
              }
              r="4"
              fill="#3b82f6"
            />
            ...
            <circle
              ...
              cy={
                height -
                padding -
                ((transitionStateEnergy - minEnergy) / energyRange) * (height - 2 * padding)
              }
              r="4"
              fill="#ef4444"
            />
            ...
            <circle
              ...
              cy={
                height -
                padding -
                ((productEnergy - minEnergy) / energyRange) * (height - 2 * padding)
              }
              r="4"
              fill="#10b981"
            />
          ...
          <line
            ...
            y1={
              height -
              padding -
              ((kineticEnergyKJ - minEnergy) / energyRange) * (height - 2 * padding)
            }
            x2={width - padding}
            y2={
              height -
      ...
      <div className="grid grid-cols-4 border-t border-gray-100">
        ...
        <div className="p-4 text-center border-r border-gray-100">
          <div className="text-2xl font-bold text-gray-900">{activationEnergy.toFixed(1)}</div>
          <div className="text-xs text-gray-600 mt-1">Activation Energy (kJ/mol)</div>
        </div>

        {/* Reaction Enthalpy */}
        <div className="p-4 text-center border-r border-gray-100">
          <div
            className={`text-2xl font-bold ${reactionEnthalpy < 0 ? 'text-green-600' : 'text-orange-600'}`}
          >
            {reactionEnthalpy > 0 ? '+' : ''}
            {reactionEnthalpy.toFixed(1)}
          </div>
          <div className="text-xs text-gray-600 mt-1">Reaction Enthalpy (kJ/mol)</div>
        </div>

        {/* Current Energy */}
        <div className="p-4 text-center border-r border-gray-100">
          <div className="text-2xl font-bold text-gray-900">{currentEnergy.toFixed(1)}</div>
          <div className="text-xs text-gray-600 mt-1">Current Energy (kJ/mol)</div>
        </div>

        {/* Reaction Progress */}
        <div className="p-4 text-center">
          <div className="text-2xl font-bold text-blue-600">{reactionProgress.toFixed(0)}%</div>
          <div className="text-xs text-gray-600 mt-1">Reaction Progress</div>
        </div>
      </div>
      ...
      {!isSimulating && (
        <div
          className={`px-4 py-2 text-center text-sm border-t ${willReact ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'}`}
        >
          {willReact ? (
            <span>
              ✅ Kinetic energy sufficient - Reaction will proceed (Probability:{' '}
              {(reactionProbability * 100).toFixed(0)}%)
            </span>
          ) : (
            <span>
              ❌ Insufficient kinetic energy - Need{' '}
              {(activationEnergy - kineticEnergyKJ).toFixed(1)} kJ/mol more
            </span>
          )}
        </div>
      )}
