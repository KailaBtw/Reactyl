 ...
 */
export const calculateAngleProbability = (
  approachAngle: number,
  reactionType: string
): AngleProbabilityResult => {
  ...
  if (reactionType.toLowerCase().includes('sn2')) {
    ...
    if (approachAngle >= 160) {
      const probability = 1.0 - ((180 - approachAngle) / 20) ** 2; // Sharp falloff
      return {
        probability,
...
export const getOptimalAngle = (reactionType: string): number => {
  if (reactionType.toLowerCase().includes('sn2')) {
    ...
    reactionType.toLowerCase().includes('sn1') ||
    reactionType.toLowerCase().includes('e1')
  ) {
...
export const isOptimalAngle = (approachAngle: number, reactionType: string): boolean => {
  ...
  const tolerance =
    reactionType.toLowerCase().includes('sn2') || reactionType.toLowerCase().includes('e2')
      ? 10
      : 45;
