...
class EnhancedMolecularPropertiesCalculator {
	 ...
	 static parseMolFile(molFileContent: string): MolecularProperties {
		 const cacheKey = this.generateCacheKey(molFileContent);
		 if (this.calculationCache.has(cacheKey)) {
			 return this.calculationCache.get(cacheKey)!;
		 }

		 const lines = molFileContent.split('\n');
		 const header = this.parseHeader(lines.slice(0, 3));
		 const counts = this.parseCountsLine(lines[3]);
		 if (!counts) throw new Error('Invalid MOL file: cannot parse counts line');
		 const atomLines = lines.slice(4, 4 + counts.atomCount);
		 const atoms = this.parseAtoms(atomLines);
		 const bondLines = lines.slice(4 + counts.atomCount, 4 + counts.atomCount + counts.bondCount);
		 const bonds = this.parseBonds(bondLines);
		 const propertiesLines = lines.slice(4 + counts.atomCount + counts.bondCount);
		 this.parsePropertiesBlock(propertiesLines, atoms);
		 
		 const properties = this.calculateAllProperties(atoms, bonds, header.name || 'Unknown');
		 this.calculationCache.set(cacheKey, properties);
		 return properties;
	 }
	 ...
	 static calculateFromMolObject(molObject: {
		 ...
		 const cacheKey = this.generateMolObjectCacheKey(molObject);
		 if (this.calculationCache.has(cacheKey)) {
			 return this.calculationCache.get(cacheKey)!;
		 }
		 ...
		 const atoms: Atom[] = molObject.atoms.map((a, idx) => {
			 ...
			 const baseMass = this.ATOMIC_MASSES[element] || 12.0;
			 return {
				 element,
		 ...
		 const bonds: Bond[] = (molObject.bonds as any[]).map((b: any) => {
			 const atom1Index = parseInt(b[0]);
			 const atom2Index = parseInt(b[1]);
			 const bondType = b[2] ? parseInt(b[2]) : 1;
			 return { atom1Index, atom2Index, bondType };
		 });
		 
		 const properties = this.calculateAllProperties(atoms, bonds, molObject.header?.name || 'Unknown');
		 this.calculationCache.set(cacheKey, properties);
		 return properties;
	 }
	 ...
	 private static generateMolObjectCacheKey(molObject: any): string {
		 ...
		 const atomData = molObject.atoms.map((a: any) => 
	 ...
	 private static parseHeader(headerLines: string[]): { name?: string, program?: string, comment?: string } {
		 return { name: headerLines[0]?.trim(), program: headerLines[1]?.trim(), comment: headerLines[2]?.trim() };
	 }
	 private static parseCountsLine(line: string): { atomCount: number, bondCount: number } | null {
		 ...
		 const bondCount = parseInt(line.substring(3, 6).trim());
	 ...
	 private static parseAtoms(atomLines: string[]): Atom[] {
		 return atomLines.map((line, index) => {
			 ...
			 if (isNaN(x) || isNaN(y) || isNaN(z)) throw new Error(`Invalid coordinates in atom line ${index + 1}`);
			 let charge = 0;
			 let stereochemistry: number | undefined;
			 let isotopeMassDiff: number | undefined;
			 if (line.length >= 39) {
				 const chargeField = parseInt(line.substring(36, 39).trim());
				 const chargeMap: Record<number, number> = { 1: 3, 2: 2, 3: 1, 5: -1, 6: -2, 7: -3 };
				 if (!isNaN(chargeField)) charge = chargeMap[chargeField] || 0;
			 }
			 if (line.length >= 36) {
				 const massDiff = parseInt(line.substring(34, 36).trim());
				 if (!isNaN(massDiff) && massDiff !== 0) isotopeMassDiff = massDiff;
			 }
			 const baseMass = this.ATOMIC_MASSES[element] || 12.0;
	 ...
	 private static parseBonds(bondLines: string[]): Bond[] {
		 return bondLines.map((line, index) => {
			 ...
			 const atom1Index = parseInt(parts[0]);
			 const atom2Index = parseInt(parts[1]);
			 const bondType = parseInt(parts[2]);
			 let stereo: number | undefined;
			 if (parts.length >= 4) {
				 const stereoField = parseInt(parts[3]);
				 if (!isNaN(stereoField) && stereoField !== 0) stereo = stereoField;
			 }
	 ...
	 private static parsePropertiesBlock(propertiesLines: string[], atoms: Atom[]): void {
		 for (const line of propertiesLines) {
			 if (line.startsWith('M  CHG')) {
				 ...
				 if (parts.length >= 4) {
					 const chargeCount = parseInt(parts[3]);
					 for (let i = 0; i < chargeCount && (4 + i * 2 + 1) < parts.length; i++) {
						 const atomIndex = parseInt(parts[4 + i * 2]) - 1;
						 const charge = parseInt(parts[4 + i * 2 + 1]);
	 ...
	 private static calculateAllProperties(atoms: Atom[], bonds: Bond[], moleculeName: string): MolecularProperties {
		 ...
		 properties.molecularFormula = this.generateMolecularFormula(atoms);

		 // Calculate center of mass (fast)
		 properties.centerOfMass = this.calculateCenterOfMass(atoms);

		 // Calculate bounding box (fast)
		 properties.boundingBox = this.calculateBoundingBox(atoms);

		 // Calculate bounding radius (fast)
		 properties.boundingRadius = this.calculateBoundingRadius(atoms, properties.centerOfMass);

		 // Calculate moments of inertia (fast)
		 properties.momentOfInertia = this.calculateMomentsOfInertia(atoms, properties.centerOfMass);
	 ...
	 private static analyzeMolecularGeometry(atoms: Atom[], bonds: Bond[], centerOfMass: THREE.Vector3): MolecularGeometry {
		 ...
		 const isLinear = this.checkLinearity(atoms, centerOfMass);
		 if (isLinear) {
			 return { 
				 type: 'linear', 
				 symmetryFactor: 2.0, 
				 principalAxes: this.calculatePrincipalAxes(atoms, centerOfMass) 
			 };
		 }

		 // Check for planarity (expensive - only do if needed)
		 const isPlanar = this.checkPlanarity(atoms, centerOfMass);
		 ...
		 if (isPlanar) {
			 type = 'planar';
			 symmetryFactor = this.calculateSymmetryFactor(atoms, bonds);
		 }
	 ...
	 private static checkLinearity(atoms: Atom[], centerOfMass: THREE.Vector3, threshold = 0.1): boolean {
	 ...
	 private static checkPlanarity(atoms: Atom[], centerOfMass: THREE.Vector3, threshold = 0.1): boolean {
		 ...
		 for (let i = 0; i < atoms.length - 2 && !found; i++) {
			 for (let j = i + 1; j < atoms.length - 1 && !found; j++) {
				 for (let k = j + 1; k < atoms.length && !found; k++) {
					 ...
					 if (cross.length() > 0.01) { // Non-collinear
	 ...
	 private static calculateSymmetryFactor(atoms: Atom[], bonds: Bond[]): number {
	 ...
	 private static calculatePrincipalAxes(atoms: Atom[], centerOfMass: THREE.Vector3): THREE.Vector3[] {
