...
export class EnhancedMolParser {
  ...
  static async convertToEnhanced(
    ...
    const enhancedAtoms: EnhancedAtom[] = molObject.atoms.map((atom, index) => ({
      ...
      hybridization: this.determineHybridization(molObject, index),
      bonds: this.findConnectedAtoms(molObject, index),
      reactivity: this.calculateAtomReactivity(molObject, index, atom.type)
    }));
    ...
    const enhancedBonds: EnhancedBond[] = molObject.bonds.map((bond, index) => ({
      ...
      order: this.determineBondOrder(bond),
      length: this.calculateBondLength(molObject, bond),
      type: this.determineBondType(bond),
      isRotatable: this.isBondRotatable(molObject, bond)
    }));
    ...
    const geometry: MolecularGeometry = {
      ...
      boundingBox: this.calculateBoundingBox(enhancedAtoms),
    ...
    const cacheMetadata: CacheMetadata = {
      ...
      parsedAt: Date.now(),
      version: '1.0',
      hash: this.generateStructureHash(molObject)
    };
   ...
   */
  private static determineHybridization(molObject: any, atomIndex: number): 'sp' | 'sp2' | 'sp3' | 'aromatic' {
    const connectedBonds = this.findConnectedAtoms(molObject, atomIndex);
   ...
   */
  private static calculateAtomReactivity(molObject: any, atomIndex: number, element: string): AtomReactivity {
    const connectedAtoms = this.findConnectedAtoms(molObject, atomIndex);
  ...
  private static determineBondType(bond: string[]): 'single' | 'double' | 'triple' | 'aromatic' {
    const order = this.determineBondOrder(bond);
   ...
   */
  private static calculateBoundingBox(atoms: EnhancedAtom[]): { min: THREE.Vector3; max: THREE.Vector3 } {
    if (atoms.length === 0) {
      return { min: new THREE.Vector3(), max: new THREE.Vector3() };
    }
    
    const positions = atoms.map(atom => new THREE.Vector3(atom.position.x, atom.position.y, atom.position.z));
