  ...
  async loadDemoMolecules(
    ...
    try {
      ...
      for (let i = 0; i < Math.min(2, demoMolecules.length); i++) {
        ...
        const positions = [
          { x: -10, y: 0, z: 0 },  // Substrate on the left
          { x: 10, y: 0, z: 0 }    // Nucleophile on the right
        ];
        ...
        log(`✅ Loaded demo molecule: ${mol.name} (${mol.formula}) at position (${positions[i].x}, ${positions[i].y}, ${positions[i].z})`);
      ...
      if (reactionType) {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            if (this.reactionCompleted || !substrate || !nucleophile || 
                substrate.reactionInProgress || nucleophile.reactionInProgress) {
