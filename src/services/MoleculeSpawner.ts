...
export class MoleculeSpawner {
  ...
  async spawnMolecule(cid: string, name: string, options: SpawnOptions = {}): Promise<any> {
    try {
      ...
      if (options.velocity) {
        velocity = new THREE.Vector3(options.velocity.x, options.velocity.y, options.velocity.z);
      ...
      const setVelocityOnBody = () => {
        ...
        if (body) {
          ...
          const angularSpeed =
            linearSpeed > 0 ? (0.5 + Math.random() * 1.5) * (linearSpeed / 10.0) : 0;
      ...
      if (Math.random() < 0.05) {
        // Log ~5% of spawns
        log(
          `✅ Spawned molecule ${name} at (${position.x.toFixed(2)}, ${position.y.toFixed(2)}, ${position.z.toFixed(2)})`
        );
      }
  ...
  async spawnMultipleMolecules(
    ...
    for (let i = 0; i < count; i++) {
      ...
      try {
        const molecule = await this.spawnMoleculeInContainer(cid, name, bounds, options);
   ...
   */
  calculateMoleculeCount(concentration: number, containerVolume: number): number {
  ...
  async spawnForConcentration(
    ...
    const containerVolume = (bounds.size / 100) ** 3; // Convert to liters (assuming units are cm)
   ...
   */
  private getRandomPositionInContainer(bounds: ContainerBounds): {
    x: number;
    y: number;
    z: number;
  } {
  ...
  async adjustConcentration(
    ...
    const containerVolume = (bounds.size / 100) ** 3;
    ...
    if (difference > 0) {
      ...
      const allMolecules = this.moleculeManager
        .getAllMolecules()
