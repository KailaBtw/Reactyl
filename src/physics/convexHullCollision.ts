...
export function cleanupAllHullMeshes(): void {
  hullMeshes.forEach((_mesh, molecule) => {
...
export function createWorldSpaceHull(molecule: MoleculeGroup): THREE.Vector3[] | null {
  ...
  try {
    ...
    console.log(
      `Created hull with ${vertices.length} unique vertices from ${points.length} atom points`
    );
    return vertices;
  } catch (error) {
...
export function visualizeHulls(scene: THREE.Scene, molecules: MoleculeGroup[]): void {
  ...
  if (!showHulls) {
    ...
    hullMeshes.forEach((mesh, _molecule) => {
...
export function checkHullIntersection(hullA: THREE.Vector3[], hullB: THREE.Vector3[]): boolean {
  if (hullA.length < 4 || hullB.length < 4) {
    console.log(
      `Hull intersection: insufficient vertices for 3D hull - A: ${hullA.length}, B: ${hullB.length}`
    );
    return false;
  }
  ...
  if (minDistance < 0.01) {
    // Much smaller threshold for "touching" (0.01 instead of 0.1)
