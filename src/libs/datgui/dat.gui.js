...
((global, factory) => {
  ...
  const Common = {
    ...
    isNaN: (_isNaN => {
      ...
      return isNaN;
    })(obj => Number.isNaN(obj)),
    isArray: Array.isArray || (obj => obj.constructor === Array),
  ...
  const inherits = (subClass, superClass) => {
  ...
  const possibleConstructorReturn = (self, call) => {
    if (!self) {
  ...
  Color.recalculateRGB = (color, component, componentHexIndex) => {
  ...
  const BooleanController = (_Controller => {
  ...
  const OptionController = (_Controller => {
  ...
  const StringController = (_Controller => {
  ...
  const NumberController = (_Controller => {
    ...
    function NumberController(object, property, params) {
      ...
      if (Common.isUndefined(_this.__step)) {
        if (_this.initialValue === 0) {
          ...
          _this.__impliedStep =
            10 ** Math.floor(Math.log(Math.abs(_this.initialValue)) / Math.LN10) / 10;
        }
      } else {
  ...
  const NumberControllerBox = (_NumberController => {
  ...
  const NumberControllerSlider = (_NumberController => {
  ...
  const FunctionController = (_Controller => {
  ...
  const ColorController = (_Controller => {
  ...
  Common.extend(GUI.prototype, {
    ...
    removeFolder: function removeFolder(folder) {
      ...
      if (this.load?.folders?.[folder.name]) {
  ...
  function addSaveMenu(gui) {
    ...
    if (gui.load?.remembered) {
      Common.each(gui.load.remembered, (_value, key) => {
  ...
  function getCurrentPreset(gui, useInitialValues) {
    const toReturn = {};
    Common.each(gui.__rememberedObjects, (_val, index) => {
      const savedValues = {};
