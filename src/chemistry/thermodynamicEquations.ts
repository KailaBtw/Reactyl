...
export function calculateKineticEnergy(
  ...
  return (0.5 * molecularMass * velocity ** 2) / 1000;
}
...
export function calculateTransitionStateRate(
  ...
  const rate =
    preExponentialFactor * Math.exp(-activationEnergy / (R * temperature)) * entropyFactor;
...
export function adjustActivationEnergyForTemperature(
  ...
  return baseActivationEnergy * temperatureRatio ** temperatureCoefficient;
}
 ...
 */
export function calculateReactionEnthalpy(bondsBroken: number[], bondsFormed: number[]): number {
