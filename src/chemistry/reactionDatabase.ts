import { ReactionType, MolecularFeature } from '../types';
...
export const REACTION_TYPES: Record<string, ReactionType> = {
  ...
  sn1: {
    ...
    probabilityFactors: {
      ...
      orientation: (angle) => 1.0 // No orientation dependence
    }
  },
...
export function isMoleculeCompatibleWithReaction(
  ...
  const requiredFeatures = role === 'substrate' 
...
export function getActivationEnergyInUnits(energy: number, unit: 'kJ/mol' | 'kcal/mol' | 'eV'): number {
  switch (unit) {
    case 'kcal/mol':
