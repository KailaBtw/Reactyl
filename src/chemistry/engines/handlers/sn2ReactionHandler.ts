...
import { type MolecularStructure, ReactionHandler, type TransitionState } from '../reactionHandler';
...
export class SN2ReactionHandler extends ReactionHandler {
  ...
  async orientMolecules(
    ...
    log(
      `SN2 orientation: electrophile C${electrophile.atomIndex}, nucleophile ${nucleophileCenter.atomIndex}, leaving group ${leavingGroup.atomIndex}`
    );
  ...
  async calculateTransitionState(
    ...
    if (
      electrophileIndex !== undefined &&
      nucleophileIndex !== undefined &&
      leavingGroupIndex !== undefined
    ) {
   ...
   */
  private findBestElectrophile(
    structure: MolecularStructure
  ): { atomIndex: number; reactivity: number } | null {
   ...
   */
  private findLeavingGroup(
    structure: MolecularStructure,
    electrophileIndex?: number
  ): { atomIndex: number; reactivity: number } | null {
  ...
  private calculateBacksideAttackVector(
    ...
    const length = Math.sqrt(
      lgVector.x * lgVector.x + lgVector.y * lgVector.y + lgVector.z * lgVector.z
    );
  ...
  private calculateStericHindrance(structure: MolecularStructure, atomIndex: number): number {
    ...
    switch (substituentCount) {
      case 1:
        return 0.0; // Methyl cation (theoretical)
      case 2:
        return 0.1; // Primary
      case 3:
        return 0.3; // Secondary
      case 4:
        return 0.9; // Tertiary (very hindered for SN2)
      default:
        return 1.0; // Impossible geometry
    }
  }
   ...
   */
  private determineAtomGeometry(
    structure: MolecularStructure,
    atomIndex: number
  ): 'tetrahedral' | 'trigonal_planar' | 'linear' | 'bent' {
    const connectedBonds = this.findConnectedBonds(structure, atomIndex);
  ...
  private modifyBondInStructure(
    ...
    const bond = structure.bonds.find(
  ...
  private calculateSN2EnergyProfile(
    ...
    for (let i = 0; i < points; i++) {
      ...
      const energy = barrier * Math.exp(-(((t - 0.5) * 4) ** 2));
      profile.push(energy);
    }
